...
class Model():
    def __init__(self, opt):
        ...
        if self.opt.gpu_ids:
            ...
            self.softmax_criteria = self.softmax_criteria.cuda()
        ...
        self.test_accuracy = Variable(torch.FloatTensor([0]), requires_grad=False)
        ...
        if self.opt.gpu_ids:
            ...
            self.test_accuracy = self.test_accuracy.cuda()
    ...
    def get_current_errors(self):
        ...
        correct_mask = torch.eq(predicted_idx, self.input_label).type(torch.FloatTensor)
        train_accuracy = torch.mean(correct_mask)

        return OrderedDict([
            ('train_loss', self.loss.data[0]),
            ('train_accuracy', train_accuracy),
            ('test_loss', self.test_loss.data[0]),
            ('test_accuracy', self.test_accuracy.data[0])
        ])
